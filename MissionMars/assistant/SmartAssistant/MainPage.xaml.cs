...
namespace SmartAssistant
{
    ...
    public enum Genre { Metal, Pop, Slow, Blues, Rock, Jazz, Electro }
    public sealed partial class MainPage : Page
    {
        ...
        private async Task InitializeRecognizer2(Language recognizerLanguage)
        {
            ...
            speechRecognizer2.UIOptions.ExampleText = "please say something..";
...

            {
                ...
                switch (res.Command)
                {
                    ...
                    case "MoveRobot":
                        ...
                        break;
                    default:
                        var call = await httpClient.GetAsync($"http://gravicodeabsensiweb.azurewebsites.net/api/AskTony/Ask?question={CommandStr}");
                        var node = JsonConvert.DeserializeObject<CustomSkill>(await call.Content.ReadAsStringAsync());
                        switch (node.action)
                        {
                            case "read":
                                //do nothing
                            ...
                            case "playmedia":
                                MediaPlayerHelper.CleanUpMediaPlayerSource(Player1.MediaPlayer);
                                Player1.MediaPlayer.Source = new MediaItem(node.actionUrl).MediaPlaybackItem;
                                Player1.MediaPlayer.Play();
                                return;
                                
                            case "mqtt":
                                //params format : host, clientid, user, pass, topic
                                var datas = node._params.Split(',');
                                if(datas.Length==5)
                                {
                                    MqttClient mqclient = new MqttClient(datas[0]);
                                    mqclient.Connect(datas[1], datas[2], datas[3]);
                                    mqclient.Publish(datas[4], Encoding.UTF8.GetBytes(node.content));
                                    await speech.Read("command has been executed");
                                }
                                else
                                {
                                    await speech.Read("the parameter is incorrect");
                                }
                                return;
                               
                            case "youtube":
                                var url = await YouTube.GetVideoUriAsync(node.actionUrl, YouTubeQuality.QualityLow);
                                MediaPlayerHelper.CleanUpMediaPlayerSource(Player1.MediaPlayer);
                                Player1.MediaPlayer.Source = new MediaItem(url.Uri.ToString()).MediaPlaybackItem;
                                Player1.MediaPlayer.Play();
                                return;
                        }
                        await speech.Read(node.content);
                        break;
                }
            });
        }
